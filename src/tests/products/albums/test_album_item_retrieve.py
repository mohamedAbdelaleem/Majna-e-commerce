...
class AlbumItemDetailTests(APITestCase):
    @classmethod
    def setUpTestData(cls) -> None:
        ...
        cls.url = reverse(
            "products:album_item",
            kwargs={"pk": cls.product.pk, "album_item_pk": cls.cover_image.pk},
        )
    ...
    def test_product_not_found_failure(self):
        url = reverse(
            "products:album_item", kwargs={"pk": 3123, "album_item_pk": self.cover_image.pk}
        )
    ...
    def test_success_album_item_detail_retrieve(self):
        response = self.client.get(self.url)
