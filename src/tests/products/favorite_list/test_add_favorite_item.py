...
class FavoriteItemCreateTests(APITestCase):
    @classmethod
    def setUpTestData(cls) -> None:
        ...
        cls.url = reverse("customers:favorite_items", kwargs={'pk': cls.customer.pk})
    ...
    def test_unauthorized_failure(self):
        ...
        url = reverse("customers:favorite_items", kwargs={'pk': self.distributor.pk})
        ...
        url = reverse("customers:favorite_items", kwargs={'pk': reviewer.pk})
        response = self.client.post(url, data=self.valid_data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)


    def test_not_same_customer_failure(self):
        url = reverse("customers:favorite_items", kwargs={'pk': self.customer.pk+1})
        response = self.client.post(url, data=self.valid_data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_Favorite_item_add_success(self):
        response = self.client.post(self.url, data=self.valid_data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    
    def test_bulk_add_Favorite_items(self):
