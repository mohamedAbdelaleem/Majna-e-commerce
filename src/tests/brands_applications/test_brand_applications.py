...
class BrandApplicationTests(APITestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.valid_file = SimpleUploadedFile(
            name="name.pdf",
            content=b"A" * settings.MAX_APPLICATION_FILE_SIZE,
            content_type="application/pdf",
        )

        cls.invalid_file_size = SimpleUploadedFile(
            name="name.pdf",
            content=b"A" * settings.MAX_APPLICATION_FILE_SIZE + 1,
            content_type="application/pdf",
        )
    ...
    def test_success_brand_application_upload(self):
        ...
        response = self.client.post(self.url, data=data, content_type='multipart/form-data')
    ...
    def test_current_inprogress_application_failure(self):
        ...
        response2 = self.client.post(self.url, data=data, content_type='multipart/form-data')
    ...
    def test_invalid_content_type_failure(self):
        data = self.valid_data
        url = reverse("brands:brand_applications", kwargs={"pk": self.brand.pk+1})
